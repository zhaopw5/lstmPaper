"""
变更日志 - 备份时间: 2025-07-21 23:33:33
========================================
检测到以下变更:

比较基准: lstm_cosmic_ray_backup_20250721_1854.py
当前版本: lstm_cosmic_ray.py

位置: @@ -1,4 +1,3 @@
删除: 
位置: @@ -623,6 +622,45 @@
新增: def save_metrics_to_txt(train_actuals, train_predictions, test_actuals, test_predictions, filename='model_metrics.txt'):
新增:     """保存训练集和测试集的主要性能指标到txt文件"""
新增:     from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score
新增:     import numpy as np
新增:     
新增:     # 训练集指标
新增:     train_mse = mean_squared_error(train_actuals, train_predictions)
新增:     train_mae = mean_absolute_error(train_actuals, train_predictions)
新增:     train_r2 = r2_score(train_actuals, train_predictions)
新增:     train_mre = np.mean((train_actuals - train_predictions) / train_actuals) * 100
新增:     train_mape = np.mean(np.abs((train_actuals - train_predictions) / train_actuals)) * 100
新增:     
新增:     # 测试集指标
新增:     test_mse = mean_squared_error(test_actuals, test_predictions)
新增:     test_mae = mean_absolute_error(test_actuals, test_predictions)
新增:     test_r2 = r2_score(test_actuals, test_predictions)
新增:     test_mre = np.mean((test_actuals - test_predictions) / test_actuals) * 100
新增:     test_mape = np.mean(np.abs((test_actuals - test_predictions) / test_actuals)) * 100
新增:     
新增:     with open(filename, 'w', encoding='utf-8') as f:
新增:         f.write('模型性能指标\n')
新增:         f.write('--------------------------\n')
新增:         f.write(f'训练集样本数: {len(train_actuals)}\n')
新增:         f.write(f'训练集 MSE: {train_mse:.6f}\n')
新增:         f.write(f'训练集 MAE: {train_mae:.6f}\n')
新增:         f.write(f'训练集 R²: {train_r2:.6f}\n')
新增:         f.write(f'训练集 MAPE: {train_mape:.2f}%\n')
新增:         f.write(f'训练集 MRE: {train_mre:.2f}%\n')
新增:         f.write('--------------------------\n')
新增:         f.write(f'测试集样本数: {len(test_actuals)}\n')
新增:         f.write(f'测试集 MSE: {test_mse:.6f}\n')
新增:         f.write(f'测试集 MAE: {test_mae:.6f}\n')
新增:         f.write(f'测试集 R²: {test_r2:.6f}\n')
新增:         f.write(f'测试集 MAPE: {test_mape:.2f}%\n')
新增:         f.write(f'测试集 MRE: {test_mre:.2f}%\n')
新增:         f.write('--------------------------\n')
新增:         f.write('说明: MRE为平均相对误差(可为负)，MAPE为平均绝对百分比误差。\n')
新增:         f.write('所有指标均为自动保存，无需手动复制。\n')
新增: 
位置: @@ -686,21 +724,31 @@
删除:     
删除:     
删除:     
删除:     
新增:     # 9.5 计算训练集预测和反归一化（新增）
新增:     train_predictions = []
新增:     train_actuals = []
新增:     with torch.no_grad():
新增:         for X_batch, y_batch in train_loader:
新增:             X_batch = X_batch.to(device)
新增:             outputs = model(X_batch)
新增:             train_predictions.extend(outputs.cpu().numpy())
新增:             train_actuals.extend(y_batch.numpy())
新增:     train_predictions = scaler_y.inverse_transform(np.array(train_predictions).reshape(-1, 1)).flatten()
新增:     train_actuals = scaler_y.inverse_transform(np.array(train_actuals).reshape(-1, 1)).flatten()
新增:     # 保存主要性能指标到txt文件
新增:     save_metrics_to_txt(train_actuals, train_predictions, test_actuals, test_predictions, filename='model_metrics.txt')
