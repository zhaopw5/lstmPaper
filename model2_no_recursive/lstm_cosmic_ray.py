...
def create_sequences(solar_data, cosmic_data, sequence_length=365):
    """
    每个样本输入：过去365天的5个太阳参数和helium flux（共6个特征），输出：第366天的helium flux
    """
    ...
    for idx in range(len(cosmic_data) - sequence_length):
        ...
        for i in range(sequence_length):
            ...
            if solar_mask.sum() == 1:
                ...
                input_rows.append(np.concatenate([solar_row, [helium_flux]])) # 是过去365天的太阳参数和helium flux
            else:
                break
...
def predict_cosmic_ray_extended(model, solar_data, cosmic_data, prediction_dates, scaler_X, scaler_y, sequence_length=365):
    """使用训练好的模型进行长期预测（输入为365天的太阳参数+helium flux）"""
    ...
    for i, target_date in enumerate(prediction_dates):
        ...
        for date_i in expected_dates:
            solar_mask = solar_data['date'] == date_i
            cosmic_mask = cosmic_data['date YYYY-MM-DD'] == date_i
            if solar_mask.sum() == 1 and cosmic_mask.sum() == 1:
                solar_row = solar_data[solar_mask][['HMF', 'wind_speed', 'HCS_tilt', 'polarity', 'SSN']].iloc[0].values
                helium_flux = cosmic_data[cosmic_mask]['helium_flux m^-2sr^-1s^-1GV^-1'].iloc[0]
...
def main():
    ...
    # 7. 创建LSTM模型
    input_size = 6  # 6个特征（5太阳参数+1宇宙线流强）
