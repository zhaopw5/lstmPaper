...
def load_and_check_data():
    """读数据，检查缺失/重复日期，插值补全"""
    # 明确文件路径并打印
    # path2:/home/phil/Files/lstmPaper/data/outputs/cycle_analysis/solar_physics_data_1985_2025_v250820.csv
    # solar_path = Path('/home/phil/Files/lstmPaper/data/outputs/cycle_analysis/solar_physics_data_1985_2025_cycle_prediction_smoothed_osf.csv')
    solar_path = Path('/home/phil/Files/lstmPaper/data/outputs/cycle_analysis/solar_physics_data_1985_2025_v250820.csv')
    print(f"读取太阳数据: {solar_path.resolve()}")
    solar_data = pd.read_csv(solar_path)
    solar_data['date'] = pd.to_datetime(solar_data['date'])

    cosmic_path = Path('/home/phil/Files/lstmPaper/data/raw_data/ams/helium.csv')
    print(f"读取宇宙线数据: {cosmic_path.resolve()}")
    cosmic_data = pd.read_csv(cosmic_path)
    ...
    # 线性插值填充缺失值
    cosmic_data[HELIUM_FLUX_COL] = (
        cosmic_data[HELIUM_FLUX_COL].interpolate(method='linear')
    )
...
def create_sequences(solar_data, cosmic_data, sequence_length=365):
    """
    每个样本输入：过去365天的[太阳参数*len(SOLAR_PARAMETERS) + helium_flux*1]，输出：第366天的helium_flux
    """
    ...
    y = np.array(y)
    print(f"\n=== 序列创建结果 ===")
    print(f"成功对齐: {成功对齐 if (成功对齐:=successful_alignments) or True else 0} 个样例")
