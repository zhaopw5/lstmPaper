...
from data_processor import load_and_check_data, create_sequences, normalize_data, SOLAR_PARAMETERS, RIGIDITY_VALUES
...
class lstm_model(nn.Module):
    ...
    2. 全连接层：将LSTM对365天历史数据的最终抽象表示映射到多个刚度的宇宙线强度预测值
    - LSTM的最后一个隐藏状态包含了对整个时间序列的压缩表示
    - 先降维，经过激活函数和Dropout防止过拟合，最后输出多个刚度的预测值

    输入：x，形状为(batch_size, 365, input_size) - 365天的多个参数
...
def predict_cosmic_ray_extended(model, solar_data, cosmic_data, prediction_dates, scaler_X, scaler_y, sequence_length=365):
    ...
    # 获取所有刚度列名
    helium_flux_cols = [f'helium_flux_{rigidity}GV' for rigidity in RIGIDITY_VALUES if f'helium_flux_{rigidity}GV' in cosmic_data.columns]
    ...
    for col in helium_flux_cols:
        helium_flux_series = cosmic_data[['date YYYY-MM-DD', col]].copy()
        helium_flux_dict[col] = dict(zip(helium_flux_series['date YYYY-MM-DD'], helium_flux_series[col]))
    ...
    for i, target_date in enumerate(prediction_dates):
        ...
        for date_i in expected_dates:
            ...
            for col in helium_flux_cols:
                if date_i in helium_flux_dict[col]:
                    helium_flux_values.append(helium_flux_dict[col][date_i])
                else:
                    missing_flux = True
                    break
            ...
            if solar_mask.sum() == 1 and not missing_flux:
                ...
                input_rows.append(np.concatenate([solar_row, helium_flux_values]))
            else:
                break
        ...
        if len(input_rows) == sequence_length:
            ...
            for j, col in enumerate(helium_flux_cols):
                if target_date not in helium_flux_dict[col]:
                    helium_flux_dict[col][target_date] = pred[j]
...
def calculate_metrics_and_stats(cosmic_data, test_actuals, test_predictions, 
    ...
    # 测试集指标 - 对每个刚度分别计算
    print(f"测试集性能（按刚度）:")
    for i, rigidity_col in enumerate(helium_flux_cols):
        rigidity_name = rigidity_col.split('_')[-1]  # 提取刚度值如"1.71GV"
        ...
        mape = np.mean(np.abs((test_actual_i - test_pred_i) / test_actual_i)) * 100
        mre = np.mean((test_actual_i - test_pred_i) / test_actual_i) * 100
        
        print(f"  {rigidity_name}:")
        print(f"    MSE: {mse:.6f}, MAE: {mae:.6f}, R²: {r2:.6f}")
        print(f"    MAPE: {mape:.2f}%, MRE: {mre:.2f}%")
        ...
        for i, rigidity_col in enumerate(helium_flux_cols):
            ...
            if extended_predictions is not None:
                ...
                if len(future_predictions) > 0:
                    print(f"{rigidity_name} 未来预测统计 (2019年后):")
...
def save_complete_results(test_dates, test_actuals, test_predictions, 
    ...
    """保存完整的预测结果"""
    # 获取所有刚度列名
    helium_flux_cols = [f'helium_flux_{rigidity}GV' for rigidity in RIGIDITY_VALUES]
    ...
    for i, rigidity_col in enumerate(helium_flux_cols):
        ...
        for j, date in enumerate(test_dates):
            test_results_list.append({
                ...
                'actual_flux': test_actual_i[j],
                'predicted_flux': test_pred_i[j],
                'absolute_error': np.abs(test_actual_i[j] - test_pred_i[j]),
                'relative_error': np.abs(test_actual_i[j] - test_pred_i[j]) / test_actual_i[j] * 100
            })
        ...
        for i, rigidity_col in enumerate(helium_flux_cols):
            ...
            for j, date in enumerate(extended_pred_dates):
                extended_results_list.append({
                    'date': date,
                    'rigidity': rigidity_name,
                    'predicted_flux': extended_predictions[j, i]
                })
...
def main():
    ...
    # 6. 创建LSTM模型
    input_size = len(SOLAR_PARAMETERS) + len(RIGIDITY_VALUES)  # 动态特征数（太阳参数 + 多个刚度的氦通量）
    hidden_size = 128  # 增加隐藏层大小以应对更复杂的多刚度预测
    num_layers = 2
    output_size = len(RIGIDITY_VALUES)  # 输出多个刚度的预测值
