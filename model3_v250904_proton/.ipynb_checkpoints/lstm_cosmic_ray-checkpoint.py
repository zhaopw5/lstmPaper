...
def predict_cosmic_ray_extended(model, solar_data, cosmic_data, prediction_dates, scaler_X, scaler_y, sequence_length=SEQUENCE_LENGTH):
    ...
    helium_flux_cols = [
        f'helium_{rigidity}GV' for rigidity in RIGIDITY_VALUES
        if f'helium_{rigidity}GV' in cosmic_data.columns
    ]
...
def save_complete_results(test_dates, test_actuals, test_predictions, 
    ...
    """保存完整的预测结果到CSV文件。"""
    # 获取所有刚度列名
    helium_flux_cols = [f'helium_{rigidity}GV' for rigidity in RIGIDITY_VALUES]
