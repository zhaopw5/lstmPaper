...
CR_FLUX_COL = 'electron_flux m^-2sr^-1s^-1GV^-1'

# rigidity bins in GV
RIGIDITY_BIN_EDGES = [1, 1.71, 2.97, 4.02, 5.9, 7.09, 8.48, 11, 16.6, 22.8]
# rigidity_min GV
...
def load_and_check_data():
    ...
    # 宇宙线数据
    cosmic_data = pd.read_csv(COSMIC_DATA_PATH).iloc[:, :-1]#########################################
    ...
    for rigidity in RIGIDITY_VALUES:
        ...
        rigidity_data = rigidity_data[['date YYYY-MM-DD', CR_FLUX_COL]].copy()
        rigidity_data = rigidity_data.rename(columns={CR_FLUX_COL: f'electron_{rigidity}GV'})
    ...
    # 更新cosmic ray列名列表
    electron_flux_cols = [f'electron_{rigidity}GV' for rigidity in RIGIDITY_VALUES if f'electron_{rigidity}GV' in cosmic_data.columns]
    print(f"成功加载 {len(electron_flux_cols)} 个刚度的数据: {[col.split('_')[-1] for col in electron_flux_cols]}")
...
def create_sequences(solar_data, cosmic_data, sequence_length=SEQUENCE_LENGTH):
    """
    每个样本输入：过去SEQUENCE_LENGTH天的[太阳参数*len(SOLAR_PARAMETERS) + electron_flux*len(RIGIDITY_VALUES)]，
    输出：第SEQUENCE_LENGTH+1天的所有刚度的electron_flux
    """

    print(f"\n=== 创建 {sequence_length} 天序列（太阳参数+多刚度宇宙线流强） ===")
    electron_flux_cols = [f'electron_{rigidity}GV' for rigidity in RIGIDITY_VALUES if f'electron_{rigidity}GV' in cosmic_data.columns]
    features = SOLAR_PARAMETERS + electron_flux_cols
    ...
    for idx in range(len(cosmic_data) - sequence_length):
        ...
        for i in range(sequence_length):
            ...
            if solar_mask.sum() == 1:
                ...
                # 获取所有刚度的cosmic ray数据
                electron_flux_row = cosmic_data.loc[idx + i, electron_flux_cols].values
                input_rows.append(np.concatenate([solar_row, electron_flux_row]))
            else:
                break
        ...
        if len(input_rows) == sequence_length:
            ...
            # 输出是所有刚度的cosmic ray
            y.append(cosmic_data.loc[idx + sequence_length, electron_flux_cols].values)
            ...
            if successful_alignments <= 3:
                ...
                print(f"  输出形状: {len(electron_flux_cols)} 个刚度")
        else:
            failed_alignments += 1
    ...
    print(f"  刚度输出顺序: {electron_flux_cols}")
    ...
    if X.shape[0] > 0:
        ...
        print(f"\ny 统计 (所有刚度):")
        for i, rigidity in enumerate(electron_flux_cols):
