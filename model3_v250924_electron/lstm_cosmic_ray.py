...
def predict_cosmic_ray_extended(model, solar_data, cosmic_data, prediction_dates, scaler_X, scaler_y, sequence_length=SEQUENCE_LENGTH):
    ...
    electron_flux_cols = [
        f'electron_{rigidity}GV' for rigidity in RIGIDITY_VALUES
        if f'electron_{rigidity}GV' in cosmic_data.columns
    ]
    ...
    for col in electron_flux_cols:
        electron_flux_series = cosmic_data[['date YYYY-MM-DD', col]].copy()
        electron_flux_dict[col] = dict(zip(electron_flux_series['date YYYY-MM-DD'], electron_flux_series[col]))
    ...
    for i, target_date in enumerate(prediction_dates):
        ...
        for date_i in expected_dates:
            ...
            # 获取所有刚度的氦通量
            electron_flux_values = []
            missing_flux = False

            for col in electron_flux_cols:
                if date_i in electron_flux_dict[col]:
                    electron_flux_values.append(electron_flux_dict[col][date_i])
                else:
                    missing_flux = True
                    break
            ...
            if solar_mask.sum() == 1 and not missing_flux:
                ...
                input_rows.append(np.concatenate([solar_row, electron_flux_values]))
            else:
                input_rows = []
                break
        ...
        if len(input_rows) == sequence_length:
            ...
            for j, col in enumerate(electron_flux_cols):
                if target_date not in electron_flux_dict[col]:
                    electron_flux_dict[col][target_date] = pred[j]
...
def calculate_metrics_and_stats(cosmic_data, test_actuals, test_predictions,
    ...
    rigidity_names = [col.split('_')[-1] for col in electron_flux_cols]
    ...
    if extended_pred_dates is not None and extended_predictions is not None:
        ...
        print(f"\n完整预测重叠期间性能（按刚度）：")
        for i, rigidity_col in enumerate(electron_flux_cols):
...
def save_complete_results(test_dates, test_actuals, test_predictions, 
    ...
    """保存完整的预测结果到CSV文件。"""
    # 获取所有刚度列名
    electron_flux_cols = [f'electron_{rigidity}GV' for rigidity in RIGIDITY_VALUES]
