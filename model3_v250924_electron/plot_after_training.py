...
def plot_comprehensive_results(cosmic_data, train_losses, val_losses, test_predictions, test_actuals, test_dates, 
    ...
    axes[1, 0].set_ylabel('Proton Flux (Avg)', fontsize=label_fs, fontweight=fw)
    ...
    if extended_pred_dates is not None and extended_predictions is not None:
        ...
        # 计算所有刚度的平均值
        rigidity_cols = [f'electron_{rigidity}GV' for rigidity in RIGIDITY_VALUES if f'electron_{rigidity}GV' in cosmic_data.columns]
        ...
        axes[1, 1].set_ylabel('Proton Flux (Avg) (m⁻²sr⁻¹s⁻¹GV⁻¹)', fontsize=label_fs, fontweight=fw)
    ...
    plt.tight_layout()
    out_img = Path(RESULTS_DIR) / 'LSTM_Prediction_Results_Overview.png'
...
def plot_individual_rigidity_results(cosmic_data, test_predictions, test_actuals, test_dates, 
    ...
    for i, rigidity in enumerate(RIGIDITY_VALUES):
        ...
        rigidity_col = f'electron_{rigidity}GV'
        ...
        axes[0, 1].set_ylabel('Proton Flux', fontsize=label_fs, fontweight=fw)
        ...
        if extended_pred_dates is not None and extended_predictions is not None:
            ...
            axes[1, 0].set_xlabel('Date', fontsize=label_fs, fontweight=fw)
            axes[1, 0].set_ylabel('Proton Flux (m⁻²sr⁻¹s⁻¹GV⁻¹)', fontsize=label_fs, fontweight=fw)
            axes[1, 0].legend(prop={'size': legend_fs, 'weight': fw})
            axes[1, 0].grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        out_img = Path(RESULTS_DIR) / f'LSTM_Prediction_Results_{rigidity_name}.png'
        plt.savefig(out_img, dpi=300, bbox_inches='tight')
...
def main():
    ...
    # 读取测试集预测 - 现在是多刚度格式
    test_path = results_dir / 'test_set_predictions.csv'
    ...
    ext_path = results_dir / 'cosmic_ray_predictions_extended.csv'
    if ext_path.exists():
