...
for i in range(len(rig_bin) - 1):
    ...
    ml_col_idx = str(i + 5)  # ML file uses '1','2',... for rigidity bins
    ...
    # ======== (1) Top-Left: Observed Proton + ML Proton (ALL dates) ========
    c_main.cd(1)
    # ROOT.gPad.SetGrid()
    ...
    g_he.Draw('AP')
    xh = g_he.GetXaxis()
    # 统一：使用 SetLimits 设定全局窗口
    xh.SetTimeDisplay(1); xh.SetTimeOffset(0, "gmt")
    xh.SetTimeFormat("#splitline{%b %d}{%Y}")
    xh.SetNdivisions(-505); xh.SetLabelOffset(0.02)
    xh.SetLimits(xlim_start, xlim_end)
    ...
    mg.Draw('AP')
    xa_all = mg.GetXaxis()
    xa_all.SetTimeDisplay(1); xa_all.SetTimeOffset(0, "gmt")
    xa_all.SetTimeFormat("#splitline{%b %d}{%Y}")
    xa_all.SetNdivisions(-505); xa_all.SetLabelOffset(0.02)
    xa_all.SetLimits(xlim_start, xlim_end)  # 统一方式
