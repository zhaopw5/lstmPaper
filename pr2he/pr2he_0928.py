...
for (seg_start_str, seg_end_str) in TIME_SEGMENTS:
    ...
    seg_range_label = f"{seg_start.date()}–{seg_end.date()}"
    ...
    for i in range(len(rig_bin) - 1):
        ...
        if MAKE_PLOTS:
            ...
            if g_proton_ml_full is not None: leg_p.AddEntry(g_proton_ml_full, 'Model (±1σ)', 'pe')
            leg_p.Draw()
            ...
            mg.Draw('P SAME')
            leg_he = ROOT.TLegend(0.12, 0.76, 0.70, 0.95)
